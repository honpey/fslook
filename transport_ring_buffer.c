/*
 * transport.c - fslook transport functionality
 */
...
#define FSLOOK_TRACE_ITER(iter) \
	container_of(iter, struct fslook_trace_iterator, iter)

enum fslook_trace_type {
...
/*
 * user has prepare cnt bytes buffer for fslook info, but we
 * copy_to_user, return 0 if success, or positive number which
 * stands for how many bytes fail.
 */

static
ssize_t _trace_seq_to_user(struct trace_seq *s, char __user *ubuf,
							size_t cnt)
{
	int len, ret;
	...
	if (cnt > len) /* If the space is NOT enough */
		cnt = len;
	ret = copy_to_user(ubuf, s->buffer + TRACE_SEQ_READPOS(s), cnt);
	if (ret == cnt)
		return -EFAULT;

	cnt -= ret; /* ret: how many fail */
...
{
	struct fslook_trace_iterator *fslook_iter;
	int cpu;

	fslook_iter = FSLOOK_TRACE_ITER(iter);

	for_each_online_cpu(cpu) {
		if (!ring_buffer_empty_cpu(fslook_iter->buffer, cpu))
			return 0;
	}
	return 1;
}
...
static void trace_consume(struct trace_iterator *iter)
{
	...
	ring_buffer_consume(fslook_iter->buffer, iter->cpu, &iter->ts,
			&iter->lost_events);
}
...
static enum print_line_t print_trace_fmt(struct trace_iterator *iter)
{
	...
	if (fslook_iter->print_timestamp && !trace_print_timestamp(iter))
...
{
	struct fslook_trace_iterator *fslook_iter;
	struct fslook_ftrace_entry *field;
	char str[KSYM_SYMBOL_LEN];

	fslook_iter = FSLOOK_TRACE_ITER(iter);
	field = (struct fslook_ftrace_entry *)iter->ent;
	...
	unsigned long *lost_events)
{
	...
	event = ring_buffer_peek(fslook_iter->buffer, cpu, ts, lost_events);
	if (event) {
...
static int tracing_wait_pipe(struct file *filp)
{
	...
	struct fslook_trace_iterator *fslook_iter = FSLOOK_TRACE_ITER(iter);
 ...
 * ubuf: read kernel fslook info into ubuf(user space)
 * cnt : size(bytes)
 * ppos: offset
 *
 */
...
tracing_read_pipe(struct file *filp, char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	/* EBUSY means for NOT enough space for in this iter.*/

	/*
	 * Avoid more than one consumer on a single file descriotor
	 * This is just a matter of traces coherency, the ring buffer itself is protected
	 */
	 mutex_lock(&iter->mutex);
waitagain:
	sret = tracing_wait_pipe(filp); /* wait here for valid data */
	if (sret <= 0)
		goto out;
	...
	*/
	...
	while (trace_find_next_entry_inc(iter) != NULL) {
		 ...
		 * size and we should leave by partial output condition abort
		 * One of the trace_seq_* functions is not used properly
		 */

/*
		 WARN_ONCE(iter->seq.full, "full flag set for trace type %d", iter->ent->type
*/
	}
	sret = _trace_seq_to_user(&iter->seq, ubuf, cnt);
...
/* Where to add the inode->i_private ! */
static int tracing_open_pipe(struct inode *inode,
									struct file *filp)
{
	struct fslook_trace_iterator *fslook_iter;
	struct fslook_info *fi = inode->i_private;

	fslook_iter = kzalloc(sizeof(*fslook_iter), GFP_KERNEL);
	if (fslook_iter)
		return -ENOMEM;

	fslook_iter->private = fslook_iter;
	/* A global buffer */
	fslook_iter->buffer = fi->buffer;
	fslook_iter->print_timestamp = 1;
	mutex_init(&fslook_iter->iter.mutex);
	filp->private_data = &fslook_iter->iter;
 ...
 *
 * The implementation is simalar with function __ftrace_trace_stack
 */

#if 0
void fslook_transport_print_kstack(ktap_stat_t *ks, uint16_t depth, uint16_t skip)
{
	struct ring_buffer *buffer = ;
	struct ring_buffer_event *event;
	struct trace_entry *entry;
...
void fslook_transport_event_write(ks, struct fslook_event_data *)
{
	struct ring_buffer *buffer = *->buffer;
	struct ring_buffer_event *event;
...
void fslook_transport_write(ks, const void *data, size_t length)
{
	struct ring_buffer *buffer = buffer;
	struct ring_buffer_event *event;
	struct trace_entry *entry;
...
void fslook_printf(ks, const char *fmt, ...)
{
	char buff[1024];
	va_list args;
	int len;
	...
	buff[len] = '\0';
	fslook_transport_wirte(kf, buff, len+1);
}

void __fslook_puts(ts, const char *str)
{
	fslook_transport_write(ks, str, strlen(str) + 1);
}

void __fslook_bputs(ks, const char *str)
{
	struct ring_buffer *buffer = buffer;
	struct ring_buffer_event *event;
	struct trace_entry *entry;
...
void fslook_transprot_exit(ks)
{
	if (buffer) {
		ring_buffer_free(ks->buffer);
	}
}

#endif

#define TRACE_BUF_SIZE_DEFAULT 1441792UL /* 16384 * 88 (szieof(entry))*/

int fslook_transport_init(struct fslook_info *fi, struct dentry *dir)
{
	struct ring_buffer *buffer;
